...
{
	t_class *c;
	retroseq_class = class_new("retroseq~", (method)retroseq_new, (method)retroseq_free, sizeof(t_retroseq), 0,A_GIMME,0);
	c = retroseq_class;
	class_addmethod(c, (method)retroseq_dsp, "dsp", A_CANT, 0);
	class_addmethod(c, (method)retroseq_assist, "assist", A_CANT, 0);
	class_addmethod(c, (method)retroseq_list, "list", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_durlist, "durlist", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_freqlist, "freqlist", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_tempo, "tempo", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_adsr, "adsr", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_sustain_amplitude, "sustain_amplitude", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_elastic_sustain, "elastic_sustain", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_shuffle_freqs, "shuffle_freqs",0);
	class_addmethod(c, (method)retroseq_shuffle_durs, "shuffle_durs",0);
	class_addmethod(c, (method)retroseq_shuffle, "shuffle",0);
	class_addmethod(c, (method)retroseq_manual_override, "manual_override", A_GIMME, 0);
	class_addmethod(c, (method)retroseq_bang, "bang",0);
	class_dspinit(c);
...
void *retroseq_new(t_symbol *s, short argc, t_atom *argv)
{
	...
	/* Create the list and bang outlets first */
	
	x->d_plist_outlet = listout((t_pxobject *)x);
	x->f_plist_outlet = listout((t_pxobject *)x);
	x->bang_outlet = bangout((t_pxobject *)x);
	x->list_outlet = listout((t_pxobject *)x);
	
	/* 
	 Now create the signal outlet. Notice that there is no signal
	...
	x->list_clock = clock_new(x,(method)retroseq_send_adsr);
	x->bang_clock = clock_new(x,(method)retroseq_send_bang);
	
	/* Allocate memory for the arrays */
	
	x->f_sequence = (float *)sysmem_newptr(MAX_SEQUENCE*sizeof(float));
	x->d_sequence = (float *)sysmem_newptr(MAX_SEQUENCE*sizeof(float) );
	x->adsr_list = (t_atom *) sysmem_newptr(10 * sizeof(t_atom));
	x->adsr_out = (float *) sysmem_newptr(10 * sizeof(float));
	x->adsr = (float *) sysmem_newptr(4 * sizeof(float));
	x->tmp_permutation = (float *)sysmem_newptr(MAX_SEQUENCE * sizeof(float));
	x->pseq_list = (t_atom *) sysmem_newptr(MAX_SEQUENCE * sizeof(t_atom));	
	
	/* Check for memory allocation failure */
	
	if(x->f_sequence == NULL || x->d_sequence == NULL){
		post("retroseq~: memory allocation fail");
...
void retroseq_free(t_retroseq *x)
{
	...
	sysmem_freeptr(x->d_sequence);
	sysmem_freeptr(x->f_sequence);
	sysmem_freeptr(x->adsr);
	sysmem_freeptr(x->adsr_out);
	sysmem_freeptr(x->adsr_list);
	sysmem_freeptr(x->tmp_permutation);
	sysmem_freeptr(x->pseq_list);
	object_free(x->list_clock);
...
void retroseq_list( t_retroseq *x, t_symbol *msg, short argc, t_atom *argv)
{
	...
	for (i=0, j=0; i < argc; i += 2, j++) {
		atom_arg_getfloat(f_sequence+j, i, argc,argv);
		atom_arg_getfloat(d_sequence+j, i+1, argc,argv);
		if(d_sequence[j] <= 0){
...
void retroseq_shuffle_durs(t_retroseq *x)
{
	...
	outlet_list(x->d_plist_outlet,NULL,d_sequence_length,pseq_list);
...
void retroseq_shuffle_freqs(t_retroseq *x)
{
	...
	outlet_list(x->f_plist_outlet,NULL,f_sequence_length,pseq_list);
...
void retroseq_durlist( t_retroseq *x, t_symbol *msg, short argc, t_atom *argv)
{
	...
	for (i=0 ; i < argc; i++) {
		atom_arg_getfloat(d_sequence+i, i, argc,argv);
	}
...
void retroseq_send_adsr(t_retroseq *x)
{
	...
	outlet_list(x->list_outlet,NULL,10,adsr_list); 
}
...
void retroseq_adsr(t_retroseq *x, t_symbol *msg, short argc, t_atom *argv)
{
	...
	if( argc < 4 ){
		error("retroseq~: not enough parameters for adsr (should be 4)");
		return;
	}
	...
	if (msg == ASSIST_INLET) {
		...
		switch (arg){
			case 0:
				...
				break;
			case 1:
				sprintf(dst,"(list) ADSR envelope");
				break;
			case 2:
				sprintf(dst,"(bang) Sequence Start Bang");
				break;
			case 3:
				sprintf(dst,"(list) Permuted Frequency List");
				break;
			case 4:
				sprintf(dst,"(list) Permuted Duration List");
				break;				
		}
	}
}
...
void retroseq_dsp(t_retroseq *x, t_signal **sp, short *count)
{
	...
	/* Attach retroseq~ to the DSP chain */
	
	dsp_add(retroseq_perform, 3, x, sp[0]->s_vec, sp[0]->s_n);
}
